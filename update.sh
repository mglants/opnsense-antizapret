...
[[ "$LISTSIZE" != "$(stat -f '%z' temp/list_orig.csv.gz)" ]] && echo "List 1 size differs" && exit 2
gunzip -fd temp/list_orig.csv.gz || exit 3
...
[[ "$LISTSIZE" != "$(stat -f '%z' temp/nxdomain.txt)" ]] && echo "List 2 size differs" && exit 2
